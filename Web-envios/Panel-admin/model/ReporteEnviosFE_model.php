		...
		public function getEnviosPorFechas(/*$inicio=false,$no_registros=false,*/$fecha1,$fecha2,$status)
		{
			...
			//{
				if($status==0 || $status==1){
					$sql = "SELECT id_envio,cliente,telefono,numeroGuia,nombreDepartamento,nombreProducto,cantidad,precio_envio,fecha,estado_entrega,pago_cargo FROM envios E INNER JOIN departamento D ON E.departamento_fk = D.id_departamento INNER JOIN productos P ON E.producto_fk = P.id_producto WHERE fecha>='$fecha1' AND fecha<='$fecha2' AND estado_entrega='$status' ORDER BY id_envio DESC"; //LIMIT $inicio,$no_registros";		
				}

				else{
					$sql = "SELECT id_envio,cliente,telefono,numeroGuia,nombreDepartamento,nombreProducto,cantidad,precio_envio,fecha,estado_entrega,pago_cargo FROM envios E INNER JOIN departamento D ON E.departamento_fk = D.id_departamento INNER JOIN productos P ON E.producto_fk = P.id_producto WHERE fecha>='$fecha1' AND fecha<='$fecha2' ORDER BY id_envio";     //DESC LIMIT $inicio,$no_registros";
				}		
			//}
